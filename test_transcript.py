"""
Test script to process an existing transcript file.
"""

import os
import sys
from pathlib import Path
from dotenv import load_dotenv
from colorama import init, Fore, Style

from utils.summarizer import ContentSummarizer

# Initialize colorama
init(autoreset=True)

# Load environment variables
load_dotenv()

# Read the transcript file
transcript_file = Path("transcripts/NASA_UFOs_-_Pagan_Rituals_Secret_Science_Time_Travel_transcript.md")

if not transcript_file.exists():
    print(f"{Fore.RED}[!] Transcript file not found: {transcript_file}{Style.RESET_ALL}")
    sys.exit(1)

print(f"{Fore.CYAN}{'='*70}")
print(f"{Fore.CYAN}Testing Summarization with Existing Transcript")
print(f"{Fore.CYAN}{'='*70}\n{Style.RESET_ALL}")

# Read transcript content
with open(transcript_file, 'r', encoding='utf-8') as f:
    transcript_content = f.read()

print(f"{Fore.GREEN}[+] Loaded transcript: {transcript_file.name}{Style.RESET_ALL}")
print(f"{Fore.CYAN}    File size: {len(transcript_content)} characters{Style.RESET_ALL}\n")

# Extract the timestamped text (skip the header)
lines = transcript_content.split('\n')
transcript_text = '\n'.join([line for line in lines if line.strip().startswith('**[')])

# Create metadata
metadata = {
    'title': 'NASA UFOs - Pagan Rituals Secret Science Time Travel',
    'uploader': 'Unknown Creator',
    'duration': 3600,  # Approximate duration
    'url': 'https://www.youtube.com/watch?v=test123'  # Test URL
}

# Test both basic and advanced modes
print(f"{Fore.YELLOW}Testing with advanced mode...{Style.RESET_ALL}\n")

# Summarize using advanced mode
summarizer = ContentSummarizer(model="gpt-4o-mini", output_dir="summaries")
summary_data = summarizer.summarize(transcript_text, metadata, analysis_mode="advanced")

print(f"\n{Fore.GREEN}[+] Summary generated successfully!{Style.RESET_ALL}")
print(f"{Fore.CYAN}    Output file: {summary_data['output_file']}{Style.RESET_ALL}\n")

print(f"{Fore.CYAN}{'='*70}")
print(f"{Fore.CYAN}Test Complete!")
print(f"{Fore.CYAN}{'='*70}\n{Style.RESET_ALL}")
